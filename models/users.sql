...
user AS (
                ...
                FROM `stitch-test-296708.mysql.User`),

rmb AS (
                SELECT userid, SUM(amounteur) AS amounteur FROM `stitch-test-296708.mysql.BetActivity` WHERE wallettype = 'RealCash' GROUP BY userid),

rmw AS (
                SELECT userid, SUM(amounteur) AS amounteur FROM `stitch-test-296708.mysql.WinActivity` WHERE wallettype = 'RealCash' GROUP BY userid)
